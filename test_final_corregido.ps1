# Test completo CRUD para Brands y Attributes - Corregido
try {
    ...
    # Paso 2: Obtener departments primero
    Write-Host "`n=== PASO 2: OBTENER DEPARTMENTS ===" -ForegroundColor Yellow
    ...
    if ($departments.Count -gt 0) {
        ...
        Write-Host "Usando department: ID=$($firstDept.id), Name=$($firstDept.name)" -ForegroundColor Green
        
        # Obtener subdepartments de este department
        Write-Host "`n=== PASO 3: OBTENER SUBDEPARTMENTS ===" -ForegroundColor Yellow
        $subdeptResponse = Invoke-WebRequest -Uri "http://127.0.0.1:8080/api/departments/$($firstDept.id)/subdepartments" -Method GET -Headers $authHeaders -UseBasicParsing
        ...
        if ($subdepartments.Count -gt 0) {
            ...
            Write-Host "No hay subdepartments disponibles para este department" -ForegroundColor Red
            return
        }
    } else {
        Write-Host "No hay departments disponibles" -ForegroundColor Red
        return
    }
    
    # Paso 4: Test POST Brand
    Write-Host "`n=== PASO 4: TEST POST BRAND ===" -ForegroundColor Yellow
    ...
    # Paso 5: Test POST Attribute
    Write-Host "`n=== PASO 5: TEST POST ATTRIBUTE ===" -ForegroundColor Yellow
    ...
    # Paso 6: Verificar totales finales
    Write-Host "`n=== PASO 6: VERIFICACION FINAL ===" -ForegroundColor Yellow
