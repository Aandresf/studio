...
import { getPermissionsMetaSync, loadPermissionsMeta, CATEGORIES_DISPLAY, PermissionMeta } from '@/lib/permissionsMeta';
...
export function PermissionsEditor({ user, onClose, onSave }: { user: any; onClose: () => void; onSave: (userId: number, perms: string[]) => Promise<void> }) {
  ...
  const [localMeta, setLocalMeta] = React.useState<PermissionMeta[]>(getPermissionsMetaSync());

  // Load permissions metadata on mount if not already cached
  React.useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        const list = await loadPermissionsMeta();
        if (!mounted) return;
        setLocalMeta(list);
      } finally {
  ...
  const groupedRaw = groupByCategory(localMeta);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        localMeta.forEach((meta: PermissionMeta) => {
  ...
  const meta = localMeta.find((m: PermissionMeta) => m.key === key);
    setChecked(s => {
      ...
      if (!value) {
        const requiredByOther = localMeta.some((m: PermissionMeta) => {
