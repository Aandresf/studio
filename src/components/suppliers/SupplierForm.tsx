...
export default function SupplierForm({ open, initial, onClose, onSaved }: any) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (initial?.id) {
        await (await import('@/lib/api')).updateSupplier(initial.id, payload);
        onSaved && onSaved({ ...initial, ...payload });
        toastSuccess('Proveedor actualizado', 'Los datos del proveedor se han guardado.');
      } else {
        const created = await (await import('@/lib/api')).createSupplier(payload);
        onSaved && onSaved(created);
        toastSuccess('Proveedor creado', 'Nuevo proveedor agregado correctamente.');
      }
      onClose && onClose();
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{initial?.id ? 'Editar Proveedor' : 'Nuevo Proveedor'}</DialogTitle>
        </DialogHeader>
