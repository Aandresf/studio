...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const startupToastsShown = React.useRef(false);

  React.useEffect(() => {
    let intervalId: any;
    const detectAndCheck = async () => {
      // If running inside Tauri, prefer a persisted bind IP recorded by the native side
      try {
        if (typeof window !== 'undefined' && (window as any).__TAURI__ && (window as any).__TAURI__.invoke) {
          try {
            ...
            if (persisted) {
              ...
              try {
                const health = await fetch(`${candidate.replace(/\/$/, '')}/api/health`, { credentials: 'include' });
                if (health.ok) {
                  // Apply as global API base so the whole frontend uses this persisted IP
                  try { setApiBase(candidate); } catch (e) { /* ignore */ }
                  // we can return early because setApiBase will be used by subsequent checks
                  return;
                }
              } catch (e) {
                // candidate not reachable, continue with normal detection
              }
            }
          } catch (e) {
  ...
  const env = typeof process !== 'undefined' ? (process.env.NEXT_PUBLIC_API_URL || '') : '';
      ...
      for (const a of attempts) {
        try {
          ...
          // prefer preferredIp or server-local ip for API base. Avoid using frontendOrigin
          // when it points back to the frontend dev server (e.g. port 9002). Use frontendOrigin
          // only as a last resort when it clearly differs from the current page origin.
          if (info) {
            const pageOrigin = typeof window !== 'undefined' && window.location ? `${window.location.protocol}//${window.location.host}` : null;
            // If preferredIp is present, prefer it (explicit server-local interface)
            if (info.preferredIp) {
              setApiBase(`http://${info.preferredIp}:` + (info.port || '3001'));
              foundBase = getApiBaseCurrent();
              break;
            }
            // fallback to engine-detected ip
            if (info.ip) {
              setApiBase(`http://${info.ip}:` + (info.port || '3001'));
              foundBase = getApiBaseCurrent();
              break;
            }
            // as last resort, use frontendOrigin only if it appears different to the page origin
            if (info.frontendOrigin && pageOrigin && info.frontendOrigin !== pageOrigin) {
      ...
      const checkHealth = async () => {
        try {
          ...
          if (r.ok) {
            ...
            // Show startup toasts (front and backend) once
            if (!startupToastsShown.current) {
              try {
                // Front URL (current window origin)
                const frontOrigin = (typeof window !== 'undefined' && window.location) ? `${window.location.protocol}//${window.location.host}` : 'http://localhost';
                toastSuccess('Front', `Front corriendo en ${frontOrigin}`);

                // Try to fetch server-info to compose backend URL
                let backendUrl = getApiBaseCurrent() || '';
                try {
                  const infoUrl = backendUrl ? `${backendUrl.replace(/\/$/, '')}/api/server-info` : '/api/server-info';
                  const infoRes = await fetch(infoUrl, { credentials: 'include' });
                  if (infoRes.ok) {
                    const info = await infoRes.json();
                    if (!backendUrl) {
                      if (info.ip && info.port) backendUrl = `http://${info.ip}:${info.port}`;
                      else if (info.frontendOrigin) backendUrl = info.frontendOrigin;
                    }
                  }
                } catch (e) {
                  // ignore
                ...
                toastSuccess('Backend', `Backend corriendo en ${backendUrl}`);
              } catch (e) {
                // ignore toast errors
              }
              startupToastsShown.current = true;
            }
            return true;
          }
        } catch (e) { /* ignore */ }
  ...
  // Detect mobile devices and redirect to the PWA route when appropriate
  React.useEffect(() => {
    try {
      const router = (null as any) as ReturnType<typeof useRouter>;
    } catch (e) {
      // noop for server
    }
  }, []);
  ...
  // Redirigir a la versión PWA en dispositivos móviles.
  React.useEffect(() => {
    try {
      if (typeof window === 'undefined') return;
      const pathname = window.location.pathname || '/';
      // Allow opt-out with ?no-pwa
      const params = new URLSearchParams(window.location.search);
      if (params.get('no-pwa')) return;

      const isAlreadyPwa = pathname.startsWith('/pwa');
      const isMobile = /Mobi|Android|iPhone|iPad|iPod|Opera Mini|IEMobile/i.test(navigator.userAgent) || (window.innerWidth && window.innerWidth <= 768);
      // If running as installed PWA, prefer the PWA route
      const isStandalone = (navigator as any).standalone || window.matchMedia('(display-mode: standalone)').matches;

      if ((isMobile || isStandalone) && !isAlreadyPwa) {
        // Use client-side navigation to preserve SPA behaviour
        try {
          const router = require('next/navigation');
          // prefer pushState to avoid full reload
          // next/navigation's useRouter cannot be called here directly because we're outside React hook scope; instead do a location replace
