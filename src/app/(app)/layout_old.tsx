...
import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { Home, ShoppingCart, Package, Box, BarChart3, Store, PanelLeft, Wifi, WifiOff } from 'lucide-react';
import { cn } from '@/lib/utils';
...
export const useBackendStatus = () => {
    const context = React.useContext(BackendStatusContext);
    if (!context) throw new Error('useBackendStatus must be used within a BackendStatusProvider');
    return context;
};
...
function SidebarNav() {
    ...
    return (
        <nav className="flex flex-col items-start px-2 text-sm font-medium lg:px-4 gap-1">
            {navItems.map((item) => (
                <Link key={item.label} href={item.href} className={cn("flex w-full items-center gap-3 rounded-lg px-3 py-2 text-muted-foreground transition-all hover:text-primary", pathname.startsWith(item.href) && "text-primary bg-muted")}>
                    <item.icon className="h-4 w-4" />
                    {item.label}
                </Link>
            ))}
    ...
    try {
        ...
        return (
            <div className="flex items-center gap-2">
                <select className="rounded border px-2 py-1 text-sm" value={userId || ''} onChange={(e) => setCurrentUser(e.target.value || null)}>
                    <option value="">(Sin usuario)</option>
                    {users.map((u: any) => <option key={u.id} value={u.id}>{u.displayName || u.username}</option>)}
                </select>
                <div className="text-xs text-muted-foreground">{loading ? 'Cargando...' : `${permissions.length} permisos`}</div>
            </div>
        );
                ...
                <div className="grid h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]">
                    ...
                    <div className="flex flex-col">
                        <header className="flex h-14 items-center gap-4 border-b bg-card px-4 lg:h-[60px] lg:px-6 z-30">
                            ...
                            <div className="ml-4"><UserSelectorInner /></div>
                        </header>
                        <main className="flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6 overflow-auto">{children}</main>
                        <StoreSelectionModal isOpen={isStoreModalOpen} onStoreSelected={(name) => { setIsStoreModalOpen(false); if (name) setActiveStoreName(name); triggerRefetch(); }} />
                    </div>
                </div>
            </CurrentUserProvider>
        </BackendStatusContext.Provider>
