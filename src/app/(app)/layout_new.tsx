...
import { getStores, getApiBaseCurrent } from '@/lib/api';
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    const check = async () => {
      try {
        // use resolved API base if available (may include /api suffix)
        const base = getApiBaseCurrent() || '';
        // normalize to ensure we call /api/health on the backend base
        let url: string;
        if (!base) {
          url = '/api/health';
