...
async function createPortableRelease() {
    try {
        ...
        // Preferir el backend ya colocado en src-tauri/resources (resultado de prepare-installer-files/tauri)
        const tauriResourcesBackend = path.join(projectRoot, 'src-tauri', 'resources', 'backend.exe');
        const fallbackBackend = path.join(backendSrcDir, '..', 'src-tauri', 'binaries', 'backend.exe');
        if (await fs.pathExists(tauriResourcesBackend)) {
            await fs.copy(tauriResourcesBackend, path.join(resourcesDir, 'backend.exe'));
        } else if (await fs.pathExists(fallbackBackend)) {
            await fs.copy(fallbackBackend, path.join(resourcesDir, 'backend.exe'));
        } else {
            console.log('Warning: no se encontró backend.exe en src-tauri/resources ni en binaries; asegúrate de compilar el backend.');
        }
...
start "Backend Console" cmd /k "cd resources && backend.exe"
:: Abrir la aplicación principal (que contiene el frontend)
...
`;
        ...
        // Antes de comprimir, copiar data runtime que Tauri/prepare-installer-files colocó en src-tauri/resources/data
        const packagedData = path.join(projectRoot, 'src-tauri', 'resources', 'data');
        if (await fs.pathExists(packagedData)) {
            console.log('Copiando data runtime a portable...');
            await fs.copy(packagedData, path.join(resourcesDir, 'data'));
        }
