...
const { hasPermission, requirePermission } = require('../lib/authorize');
const router = express.Router();
const databaseManager = require('../database-manager');
const util = require('util');

// GET / - list products with variants and attribute values (public read for PWA)
// NOTE: kept other mutating endpoints protected; this read endpoint allows unauthenticated access for PWA devices.
router.get('/', async (req, res) => {
  // Allow public read for PWA clients that send header x-pwa: '1'
  const isPwa = (req.headers['x-pwa'] === '1');
  if (!hasPermission(req, 'products:read') && !isPwa) {
    return res.status(403).json({ error: 'Forbidden: missing permission products:read' });
  }
  try {
...
// GET /:id/movements - product movements (allow PWA public read)
