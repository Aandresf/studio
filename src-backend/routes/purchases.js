...
router.get('/', async (req, res) => {
    const rawPwa = req.get && req.get('x-pwa');
    const isPwa = rawPwa && (String(rawPwa).trim() === '1' || String(rawPwa).toLowerCase() === 'true');
    if (!hasPermission(req, 'purchases:read') && !isPwa) {
        return res.status(403).json({ error: 'Forbidden: missing permission purchases:read' });
    }
    try {
        const db = databaseManager.getActiveDb();
...
router.get('/details', async (req, res) => {
    const rawPwa = req.get && req.get('x-pwa');
    const isPwa = rawPwa && (String(rawPwa).trim() === '1' || String(rawPwa).toLowerCase() === 'true');
    // allow public read for PWA requests
    if (!isPwa && !hasPermission(req, 'purchases:read')) {
        return res.status(403).json({ error: 'Forbidden: missing permission purchases:read' });
    }
